...
struct led_dev{
    ...
    spinlock_t          lock;       /* 锁 */
    int                 state;      /* 设备状态：0未使用 >0使用中*/
};

static struct led_dev leddev = {
    .lock = ATOMIC_INIT(1),         /* 默认初始化为1 */
};
...
{
    unsigned long flags;
    /* 自旋锁的使用应尽可能的短，所以添加一个状态字来判断是否被使用，在加锁后只操作那个状态字 */
    spin_lock_irqsave(&leddev.lock, flags); /* 加锁 */
    if(leddev.state) {               /* 设备在使用中，返回设备忙 */
