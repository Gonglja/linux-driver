...
static int __init led_init(void) {
    ...
    /* 4. 获取reg属性内容 */ 
    ret = of_property_read_u32_array(leddev.nd, "reg", regdata, 4);
    if( ret < 0) {
        printk("reg property read failed!\r\n");
    } else {
        printk("reg data:\r\n");
        for(int i=0;i<sizeof(regdata)/sizeof(regdata[0]); ++i) {
            printk("%#X ", regdata[i]);
        }
        printk("\r\n");
    }
...
#else
    GPJ2CON_VA    = of_iomap(leddev.nd, 0);
    GPJ2DAT_VA    = of_iomap(leddev.nd, 1);
#endif
