...
static ssize_t key_read(struct file *file, char __user *buf, size_t size, loff_t *offset) 
{
...
#if 0
    ret = wait_event_interruptianle(dev->r_wait,
    ...
    if(atomic_read(&dev->keyirq[0].releasekey) == 0) { /* 没有按键按下 */
...
wait_error:
    set_current_state(TASK_RUNNING);       /* 设置任务未运行态 */
    remove_wait_queue(&dev->r_wait, &wait);  /* 将等待队列移除   */
    return ret;
...
void timer_callback(struct timer_list *tl) 
{
    ...
    /* 唤醒进程 */
    if(atomic_read(&key->releasekey)) { /* 完成一次按键过程 */
        /* wake_up(&dev->r_wait) */
